...
func WriteWatcherURL(filePath string, watcher WatcherURL) error {
	// load the content of json file
	watchers, err := ReadWatcherURLs(filePath)
	// todo:
	fmt.Printf("%p ", &watchers)
	fmt.Printf("%p\n", watchers)
	if err != nil {
...
// Returns error if reading, marshalling or writing fails.
func WriteItemIDs(filePath string, items []ItemID) error {
	...
	updatedContent, err := json.MarshalIndent(itemsToWrite, "", "  ")
	if err != nil {
